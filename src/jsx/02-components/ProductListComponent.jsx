...
import { MOVE_LIST_PRODUCT_ACTION, DELETE_PRODUCT_ACTION } from '../../redux/action'

const ProductListComponent = ({id, name, quantity, price, ready}) => {
    ...
    return(
        <form className="list__product" onSubmit = { deleteTask }>
            <p className="list__product-name">{name}</p>

            <div className="list__product-actions">
                <input className="list__product-cart" title="Move to cart list" type="checkbox" checked={checked}
                    onChange = { (e)=>setChecked(e.target.checked) }
                    onClick = { ()=>dispatch( MOVE_LIST_PRODUCT_ACTION({id,name,quantity,price,ready}) ) }
                />
                <button className="list__product-delete" type="submit" title="Delete product">
            ...
            <p className="list__product-subtotal">
                <span className="list__product-quantity">{quantity}</span>
                <span className="list__product-price">{price}</span>
                <span className="list__product-total">{quantity*price}</span>
            </p>
        </form>
    )
}
